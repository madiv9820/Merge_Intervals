...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        intervals: List[int] = testcase['input']['intervals']
        ...
        try:
            actual_output: int = self._run_case(intervals=intervals)
        except TimeoutError:
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, intervals: List[List[int]]) -> List[List[int]]:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        return self.__solution.merge(intervals=intervals)

class MergedIntervalsTestResult(unittest.TextTestResult):
    ...
    def stopTestRun(self):
        ...
        print('\n' + '🧾' + '=' * 58)
        print('📚 Merge Intervals Final Summary')
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=MergedIntervalsTestResult)
    )
